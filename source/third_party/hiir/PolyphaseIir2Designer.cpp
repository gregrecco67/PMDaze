...
int PolyphaseIir2Designer::compute_nbr_coefs_from_proto(double attenuation,
                                                        double transition) noexcept
{
...
double PolyphaseIir2Designer::compute_atten_from_order_tbw(int nbr_coefs,
                                                           double transition) noexcept
{
    assert(nbr_coefs > 0);
    assert(transition > 0);
...
int PolyphaseIir2Designer::compute_coefs(double coef_arr[], double attenuation,
                                         double transition) noexcept
{
    assert(attenuation > 0);
    assert(transition > 0);
...
void PolyphaseIir2Designer::compute_coefs_spec_order_tbw(double coef_arr[], int nbr_coefs,
                                                         double transition) noexcept
{
    assert(nbr_coefs > 0);
    assert(transition > 0);
...
PolyphaseIir2Designer::ResCode PolyphaseIir2Designer::compute_coefs_spec_order_pdly(
    double coef_arr[], double *attenuation_ptr, double *transition_ptr, int nbr_coefs,
    double phase_delay, double f_rel, double prec, double atten_lb, double atten_ub,
    double trans_lb, double trans_ub) noexcept
{
    using namespace std::placeholders;

    return compute_coefs_spec_order_delay(
        coef_arr, attenuation_ptr, transition_ptr, nbr_coefs, phase_delay, f_rel, prec, atten_lb,
        atten_ub, trans_lb, trans_ub, std::bind(compute_phase_delay, _1, _2, _3, false));
}
...
PolyphaseIir2Designer::ResCode PolyphaseIir2Designer::compute_coefs_spec_order_gdly(
    double coef_arr[], double *attenuation_ptr, double *transition_ptr, int nbr_coefs,
    double group_delay, double f_rel, double prec, double atten_lb, double atten_ub,
    double trans_lb, double trans_ub) noexcept
{
    using namespace std::placeholders;

    return compute_coefs_spec_order_delay(
        coef_arr, attenuation_ptr, transition_ptr, nbr_coefs, group_delay, f_rel, prec, atten_lb,
        atten_ub, trans_lb, trans_ub, std::bind(compute_group_delay, _1, _2, _3, false));
}
...
double PolyphaseIir2Designer::compute_phase_delay(const double coef_arr[], int nbr_coefs,
                                                  double f_fs, bool ph_flag) noexcept
{
    ...
    {
        const auto dly = compute_unit_phase_delay(coef_arr[k], f_fs, ph_flag);
        dly_arr[k & 1] += dly;
    }
...
double PolyphaseIir2Designer::compute_group_delay(const double coef_arr[], int nbr_coefs,
                                                  double f_fs, bool ph_flag) noexcept
{
    assert(coef_arr != nullptr);
    assert(nbr_coefs > 0);
    ...
    {
        const auto dly = compute_unit_group_delay(coef_arr[k], f_fs, ph_flag);
        dly_total += dly;
    }
...
void PolyphaseIir2Designer::compute_transition_param(double &k, double &q,
                                                     double transition) noexcept
{
    assert(transition > 0);
    assert(transition < 0.5);
...
template <typename F>
PolyphaseIir2Designer::ResCode PolyphaseIir2Designer::compute_coefs_spec_order_delay(
    double coef_arr[], double *attenuation_ptr, double *transition_ptr, int nbr_coefs, double delay,
    double f_rel, double prec, double atten_lb, double atten_ub, double trans_lb, double trans_ub,
    F compute_delay) noexcept
{
    assert(nbr_coefs > 0);
    ...
    if (ret_val == ResCode_OK)
    {
        ...
        do
        {
            ...
            {
                ub_tbw = rs_tbw;
                //				ub_gdly = rs_gdly; // ub_gdly not used in the loop
                // actually
            }
            else
            {
        ...
        else if (rs_attn < atten_lb || rs_attn > atten_ub)
        {
