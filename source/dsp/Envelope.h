...
class Envelope
{
  public:
    ...
    struct Params
    {
        ...
        Params(double attackTimeMs_, double decayTimeMs_, double sustainLevel_,
               double releaseTimeMs_,
               double aCurve_, // 0 = linear, 1 = convex, -1 = concave
               double dRcurve_, bool repeat_, bool sync_ = false, float syncduration_ = 1.f)
            : attackTimeMs(attackTimeMs_), decayTimeMs(decayTimeMs_), sustainLevel(sustainLevel_),
              releaseTimeMs(releaseTimeMs_), aCurve(aCurve_), dRCurve(dRcurve_), repeat(repeat_),
              sync(sync_), syncduration(syncduration_)
        {
        }

        double attackTimeMs{1.0}, decayTimeMs{1.0}, sustainLevel{0.5}, releaseTimeMs{1.0},
            aCurve{1.0}, dRCurve{-1.0};
  ...
  private:
    ...
    double linearIdxVal{0.0}, attackRate{0.0}, decayRate{0.0}, releaseRate{0.0}, finalOut{0.0},
        releaseStart{0.0};
