...
//==============================================================================
PMEditor::PMEditor(PMProcessor &p) : ProcessorEditor(p), proc(p)
{
    setName("Editor");
...
bool PMEditor::keyPressed(const juce::KeyPress &key, Component * /*originatingComponent*/)
{
    ...
    if (key.isKeyCode(juce::KeyPress::escapeKey) || key.isKeyCode(76))
    { // "L" for learning
...
void PMEditor::showAboutInfo()
{
    ...
    auto w = std::make_shared<gin::PluginAlertWindow>("---- Info ----", msg,
...
//==============================================================================
void PMEditor::paint(juce::Graphics &g) { ProcessorEditor::paint(g); }

void PMEditor::resized()
{
...
void PMEditor::addMenuItems(juce::PopupMenu &m)
{
    m.addSeparator();
    m.addItem("MPE", true, proc.globalParams.mpe->getUserValueBool(), [this] {
        proc.globalParams.mpe->setUserValue(proc.globalParams.mpe->getUserValueBool() ? 0.0f
                                                                                      : 1.0f);
    });
